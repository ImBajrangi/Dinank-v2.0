...
interface VrindopnishadLogoProps extends SvgProps {
  size?: number;
  color?: string;
  dotColor?: string;
}
...
/**
 * Official Brand Logo of Vrindopnishad
 * Renders the vector mark with high-contrast theme adaptability
 */
export const VrindopnishadLogo: React.FC<VrindopnishadLogoProps> = ({
  size = 40,
  color,
  dotColor,
  ...props
}) => {
  const { isDark, colors } = useBirthdays();

  const mainColor = color || (isDark ? '#FFFFFF' : '#0F172A');
  const rajColor = dotColor || '#FFDE00';
  const ellipseColor = isDark ? '#FFFFFF' : (color || colors.accent || '#007AFF');
